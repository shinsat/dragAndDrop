...
class MyListWidget(QListWidget):
    ...
    def mousePressEvent(self, QMouseEvent):
        ...
        if self.currentRow() == 1:
            mime = QMimeData()
            mime.setData('text/plain', b'GotIt!')
            #mime.setText
            drag = QDrag(self)
            drag.setMimeData(mime)
            drag.exec(Qt.MoveAction)
            #drag.start(Qt.CopyAction | Qt.CopyAction | Qt.MoveAction)

            #super(MyListWidget, self).startDrag(dropAction)
...
class Form(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        listWidget = MyListWidget() #QListWidget()
